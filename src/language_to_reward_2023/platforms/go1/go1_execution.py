...
"""Execute untrusted code to get task parameters for the Go1 MJPC task."""
...
from language_to_reward_2023.platforms.go1 import go1_l2r_tasks


class Go1SafeExecutor(metaclass=abc.ABCMeta):
  """Runs untrusted code to get task parameters for the Go1 MJPC task."""
  ...
  def execute(self, code: str) -> mjpc_parameters.MjpcParameters:
    ...
    Returns:
      MJPC task parameters for the Go1 robot.
    """
...
def _parse_output(output: str) -> mjpc_parameters.MjpcParameters:
  ...
  weights, params = go1_l2r_tasks.defaults()
...
def {function.__name__}(*args, **kwargs):
  ...
  go1_l2r_tasks.{function.__name__}(weights, params, *args, **kwargs)
...
_SUPPORTED_FUNCTIONS = (
    go1_l2r_tasks.set_torso_targets,
    go1_l2r_tasks.set_foot_pos_parameters,
    go1_l2r_tasks.set_foot_stepping_parameters,
    go1_l2r_tasks.set_target_joint_angles,
    go1_l2r_tasks.head_towards,
    go1_l2r_tasks.walk,
)
