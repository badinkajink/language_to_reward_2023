...
from language_to_reward_2023.platforms.go1 import go1_execution
from language_to_reward_2023.platforms.go1 import go1_l2r_tasks


def _make_confirmation_executor():
  return go1_execution.Go1SafeExecutor(
...
class go1ExecutionTest(parameterized.TestCase):
  ...
  def test_empty_code_returns_default_weights(self, executor_builder):
    ...
    default_cost_weights, default_task_params = go1_l2r_tasks.defaults()
  ...
  def test_invalid_parameters_ignored(self, executor_builder):
    executor = executor_builder()
    _, default_task_params = go1_l2r_tasks.defaults()
...
"""
  ...
  def test_all_supported_functions_are_valid(self):
    for f in go1_execution._SUPPORTED_FUNCTIONS:
