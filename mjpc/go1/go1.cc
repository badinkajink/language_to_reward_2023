...
//  ============  transition  ============
void Go1Flat::TransitionLocked(mjModel* model, mjData* data) {
  // ---------- handle mjData reset ----------
...
void Go1Flat::ModifyScene(const mjModel* model, const mjData* data,
...
void Go1Flat::ResidualFn::AverageFootPos(
...
double Go1Flat::ResidualFn::GetPhase(double time) const {
  return phase_start_ + (time - phase_start_time_) * phase_velocity_;
}

// horizontal Walk trajectory
void Go1Flat::ResidualFn::Walk(double pos[2], double time) const {
...
// get gait
Go1Flat::ResidualFn::A1Gait Go1Flat::ResidualFn::GetGait() const {
  if (current_mode_ == kModeBiped)
    return kGaitTrot;
...
double Go1Flat::ResidualFn::StepHeight(double time, double footphase,
...
void Go1Flat::ResidualFn::FootStep(double step[kNumFoot], double time,
                                         A1Gait gait) const {
...
// height during flip
double Go1Flat::ResidualFn::FlipHeight(double time) const {
...
void Go1Flat::ResidualFn::FlipQuat(double quat[4], double time) const {
  double angle = 0;
  if (time >= jump_time_ + flight_time_ + land_time_) {
    angle = 2*mjPI;
...
// -----------------------------------------------------------------------
void Go1Hill::ResidualFn::Residual(const mjModel* model,
                                         const mjData* data,
                                         double* residual) const {
  // ---------- Residual (0) ----------
...
// -----------------------------------------------
void Go1Hill::TransitionLocked(mjModel* model, mjData* data) {
